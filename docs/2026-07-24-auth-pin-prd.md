...
| 상태 | v1.0(ID+PIN) 배포 완료 · v1.1(부관리자·사역표 적용) 배포 완료 |
...
| **부관리자** (v1.1) | 일반 사용자 권한 + **지정된 영역**의 관리 권한. 영역별(사역표·자재소모) **각 1명**, 주관리자가 지정. (§13) |
| 관리자(`admin`) | 위 모든 것 + **타 사용자 PIN 초기화·삭제**, **부관리자 지정**, 로그 조회, 전체삭제. |
...
## 13. 확장 — 부관리자 역할 및 페이지 적용 (v1.1)

주관리자(`admin`)가 **자재소모·사역표 두 영역의 담당 부관리자를 그때그때 지정**해 운영한다.

### 13.1 역할 저장 (Firebase 규칙 변경 불필요)
- 사용자 기록에 역할 플래그를 둔다: `users/{id}/dutyAdmin`(사역표), `users/{id}/materialAdmin`(자재소모) — boolean.
- `users` 경로는 이미 읽기·쓰기가 열려 있어 **추가 규칙/콘솔 작업 없이** 사용 가능. (앱 레벨 권한, §9와 동일한 신뢰-내부팀 수준)
- **영역당 딱 1명:** 지정 시 대상만 `true`, 나머지 전원 해당 필드 `false`로 갱신(자동 해제).

### 13.2 지정 UI — `admin.html` (주관리자 전용)
- 회원 목록 각 행에 **[사역표] [자재소모]** 토글. 켜면 기존 담당자 자동 해제. `admin` 행은 "주관리자"로 표시(토글 없음).
- 변경 시 로그 `ADMIN_SET_ROLE` 기록.

### 13.3 사역표 페이지(`duty_roster.html`) 적용
- 로그인 강제(기존 유지).
- **관리 권한** `canManage = (id==='admin') || (내 기록.dutyAdmin===true)` — 인원 추가/삭제·순서변경·순번지정·관리 열 표시를 이 기준으로 통제.
- **"✅ 완료 → 다음 순번"**: 이번 순번 **본인**(사역표 인원 이름 = 로그인 아이디) 또는 관리자/사역표 부관리자만 가능. 그 외에는 버튼 비활성.
- 사역표엔 "작업자별 개인 기록"이 없어 자재소모식 '본인 기록 삭제' 개념은 없음 — 인원 삭제 등은 모두 '관리' 동작(canManage).

### 13.4 자재소모 페이지 — 예정
- `remove_list.html`에 로그인 강제 + `canManageMaterial = (id==='admin') || materialAdmin` 기반 삭제 권한(개별=작성자/관리, 월 전체=관리)을 적용할 예정. (본 문서 시점 미적용, 다음 작업)

### 13.5 인수 조건 (v1.1)
- [x] 주관리자가 사역표/자재소모 부관리자를 각 1명 지정, 재지정 시 기존자 자동 해제.
- [x] 사역표 부관리자는 사역표 관리(추가/삭제/순번) 가능, 일반 사용자는 불가.
- [x] "완료" 버튼은 이번 순번 본인 또는 관리자/부관리자만 활성.
- [x] Firebase 규칙 변경 없이 동작.
